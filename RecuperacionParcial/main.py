...
class Resistenciasserie(QMainWindow):
    ...
    def calcularserie(self):
        ...
        if resis1s <= 0 or resis2s <= 0 or resis3s <= 0:
            QMessageBox.information(self, "Error", "Las resistencias no pueden ser menores o iguales a 0")
        else:   
...
class Resistenciasparalelo(QMainWindow):
    ...
    def calcularparalelo(self):
        ...
        if resis1p <= 0 or resis2p <= 0 or resis3p <= 0 or resis4p <= 0:
            QMessageBox.information(self, "Error", "Las resistencias no pueden ser menores o iguales a 0") 
        else: 
            p1 = 1 / resis1p
            p2 = 1 / resis2p
...
class Resistenciasserieyparalelo(QMainWindow):
    ...
    def calcularserieyparalelo(self):
        ...
        if resis1pys <= 0 or resis2pys <= 0 or resis3pys <= 0 or resis4pys <= 0 or resis5pys <= 0:
            QMessageBox.information(self, "Error", "Las resistencias no pueden ser menores o iguales a 0") 
        else:
